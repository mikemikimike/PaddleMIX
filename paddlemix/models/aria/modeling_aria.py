...
def build_mm_projector(config: AriaConfig):
    ...
    """
    return AriaProjector(patch_to_query_dict=config.
        projector_patch_to_query_dict, embed_dim=config.vision_config.
        hidden_size, num_heads=config.vision_config.num_attention_heads,
        kv_dim=config.vision_config.hidden_size, ff_dim=config.text_config.
        hidden_size, output_dim=config.text_config.hidden_size)
...
class AriaForConditionalGeneration(AriaPretrainedModel, GenerationMixin):
    ...
    def forward(self, input_ids: paddle.Tensor=None, pixel_values: paddle.
        Tensor=None, pixel_mask: paddle.Tensor=None, attention_mask:
        ...
        cache_position: Optional[paddle.Tensor]=None, num_logits_to_keep: int=0
        ...
        if pixel_values is not None:
            image_outputs, image_attn_mask = self.vision_tower(pixel_values,
                pixel_mask=pixel_mask)
            selected_image_feature = image_outputs.last_hidden_state
            image_features = self.multi_modal_projector(selected_image_feature,
                attn_mask=image_attn_mask)
        if image_features is not None:
            n_image_tokens = (input_ids == self.config.image_token_index).sum(
                ).item()
            n_image_features = tuple(image_features.shape)[0] * tuple(
            ...
            special_image_mask = (input_ids == self.config.image_token_index
                ...
                inputs_embeds.place)
            image_features = image_features.to(inputs_embeds.place,
                inputs_embeds.dtype)
            ...
            inputs_embeds = inputs_embeds.masked_scatter_(special_image_mask, image_features) #ppdiffusers/examples/PhotoMaker/photomaker/model.py 的FuseModule 也那么干了
            ...
            output_hidden_states, return_dict=return_dict, cache_position=
            cache_position, num_logits_to_keep=num_logits_to_keep)
        logits = outputs[0]
        loss = None
        if labels is not None:
            if attention_mask is not None:
                shift_attention_mask = attention_mask[:, -(tuple(logits.
                    shape)[1] - 1):].to(logits.place)
                shift_logits = logits[..., :-1, :][shift_attention_mask.to(
                    logits.place) != 0].contiguous()
                shift_labels = labels[..., 1:][shift_attention_mask.to(
                    labels.place) != 0].contiguous()
            else:
        ...
        return AriaCausalLMOutputWithPast(loss=loss, logits=logits,
            past_key_values=outputs.past_key_values, hidden_states=outputs.
            hidden_states, attentions=outputs.attentions)

    def prepare_inputs_for_generation(self, input_ids, past_key_values=None,
        inputs_embeds=None, pixel_values=None, pixel_mask=None,
        attention_mask=None, cache_position=None, num_logits_to_keep=None,
        **kwargs):
        model_inputs = self.language_model.prepare_inputs_for_generation(
            input_ids, past_key_values=past_key_values, inputs_embeds=
            inputs_embeds, attention_mask=attention_mask, cache_position=
